...
import { defaultValues, formSchema, FormSchema } from "./FormEditElement.form";
import { FormEditElementProps } from "./FormEditElement.types";

const FormEditElement = ({ dataElement }: FormEditElementProps) => {
  ...
  const form = useForm<FormSchema>({
    ...
    defaultValues: {
      ...defaultValues,
      ...Object.fromEntries(
        Object.entries(dataElement).map(([key, value]) => [key, value ?? ""])
      ),
    },
  });
  ...
  const onSubmit: SubmitHandler<FormSchema> = async (data) => {
    try {
      await axios.patch(`/api/items/${dataElement.id}`, data);
